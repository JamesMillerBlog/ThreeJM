...
var Reticulum = (function () {
    ...
    fuse.initiate = function( options ) {
        ...
        this.mesh.position.z = 0.0001; // Keep in front of reticle MAKE DYNAMIC FROM RAYCAST?
        this.mesh.rotation.y = 180*(Math.PI/180); //Make it clockwise THIS NORMAL MAP?
        //Add to reticle
    ...
    reticle.setDepthAndScale = function( depth ) {
        ...
        var cameraZ;
        if(self.flyControls) cameraZ =  settings.camera.position.z;
        else cameraZ =  self.sphereBody.position.z;
        ...
        // if not staring at anything
        if(Reticulum.retDistance().gaze == 0) {
            crosshair.scale.set( z, z, scale );
            // console.log(crosshair.scale);
        }
        else {
            // console.log(crosshair.scale);
            if(scale < 30) crosshair.scale.set( 1.5, 1.5, scale );
            else crosshair.scale.set( 1.5, 1.5, 30 );
        }
    };
    ...
    var detectHit = function() {
        try {
            this.raycaster.setFromCamera( vector, settings.camera );
        } catch (e) {
            //Assumes PerspectiveCamera for now... 
            //Support for Three.js < rev70
            // console.log(settings.camera.position);
            this.raycaster.ray.origin.copy( settings.camera.position );
            this.raycaster.ray.direction.set( vector.x, vector.y, 0.5 ).unproject( settings.camera ).sub( settings.camera.position ).normalize();

        }

        //
        var intersects = this.raycaster.intersectObjects(collisionList);
        var intersectsCount = intersects.length;
        hitTrue = intersectsCount;

        for ( var i = 0; i < intersects.length; i++ ) distance = intersects[0].distance;
    ...
    var gazeLong = function( threeObject ) {
        ...
        if( reticle.active ) {
            ...
            if(!settings.lockDistance){
                ...
                if(!self.flyControls) distance = self.sphereBody.position.distanceTo( reticle.worldPosition );
                else distance = self.camera.position.distanceTo( reticle.worldPosition );
                distance -= threeObject.geometry.boundingSphere.radius;
            }

            reticle.hit = true;
            if(!settings.lockDistance) reticle.setDepthAndScale(distance/5);
        }
        ...
        if( gazeTime >= fuse.duration && !fuse.active ) {
            ...
            //Does object have an action assigned to it?
            if (threeObject.onGazeLong != null && !gazedLong) {
